...
const id = process.argv[2] ?? 'gun_maintenance_bench';
assert(['gun_maintenance_bench','precision_fabrication_station'].includes(id));
if (id === 'gun_maintenance_bench') await import('./validate-gun-maintenance-bench.mjs');
else await import('./sync-precision-fabrication-station.mjs');
...
const full = read(root + `models/block/${id}.json`);
...
for(const [name,elements] of Object.entries(output)) {
  write(root+`models/block/${id}/${name}.json`,{textures:full.textures,elements});
}
const variants={};
for(const [facing,y] of [['north',0],['east',90],['south',180],['west',270]]) for(const [name] of parts)
  variants[`facing=${facing},part=${name}`]={model:`apocalypse_firstlight:block/${id}/${name}`,y};
write(root+`blockstates/${id}.json`,{variants});
write(root+`models/item/${id}.json`,{parent:`apocalypse_firstlight:block/${id}`});
