...
const model = read(base + 'models/block/gun_workbench.json');
const geoPath = base + 'geo/gun_workbench.geo.json';
...
const png = fs.readFileSync(base + 'textures/block/gun_workbench.png');
assert.equal(png.readUInt32BE(16), 256);
assert.equal(png.readUInt32BE(20), 256);
assert(png.equals(Buffer.from(source.textures[0].source.split(',')[1], 'base64')));
assert.equal(model.textures['0'], 'apocalypse_firstlight:block/gun_workbench');
...
console.log(JSON.stringify({asset: 'gun_workbench', cubes: source.elements.length,
  ...
  inGameValidation: 'Asset-only check; see gun_workbench_block_v1.md for runtime verification'}, null, 2));
