...
const base=n=>s.animations.find(a=>a.name==='animation.p9_01.'+n);
const fire=base('fire'), reload=base('reload');
assert(fire && reload);
const clone=(a,n)=>{const c=structuredClone(a); c.name='animation.p9_01.'+n;c.uuid=randomUUID();c.selected=false;
...
for(const [p,value]of outputs())if(p.endsWith('p9_01.animation.json'))fs.writeFileSync(p,JSON.stringify(value,null,2)+'\n');
