...
public final class StaticWorkstationIntegrity {
    private StaticWorkstationIntegrity() {}
    @SubscribeEvent public static void onLoad(ChunkEvent.Load event) {
        ...
        level.getServer().execute(() -> {
            ...
            for (int i=0;i<sections.length;i++) {
                ...
                if (!section.maybeHas(s -> s.getBlock() instanceof StaticWorkstationBlock)) continue;
                int bottom = level.getMinBuildHeight() + i * 16;
                for (int y=0;y<16;y++) for (int z=0;z<16;z++) for (int x=0;x<16;x++) {
                    var block = section.getBlockState(x,y,z).getBlock();
                    if (block instanceof StaticWorkstationBlock)
