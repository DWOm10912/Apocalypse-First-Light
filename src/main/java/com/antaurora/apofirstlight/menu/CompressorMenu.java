...
public final class CompressorMenu extends AbstractContainerMenu {
    private static final MachineGuiLayout LAYOUT = MachineGuiLayouts.compressor();
    private static final int MACHINE_SLOT_COUNT = CompressorBlockEntity.CONTAINER_SIZE;
    ...
    public CompressorMenu(int containerId, Inventory inventory, FriendlyByteBuf buffer) {
        this(containerId, inventory, requireBlockEntity(inventory, buffer),
                new SimpleContainerData(CompressorBlockEntity.DATA_COUNT));
    }

    public CompressorMenu(int containerId, Inventory inventory, CompressorBlockEntity compressor,
                          ContainerData data) {
        super(AflMenus.COMPRESSOR.get(), containerId);
        checkContainerSize(compressor, CompressorBlockEntity.CONTAINER_SIZE);
        checkContainerDataCount(data, CompressorBlockEntity.DATA_COUNT);
        this.container = compressor;
        this.data = data;

        MachineGuiLayout.Element inputSlot = LAYOUT.element("input_slot");
        addSlot(new Slot(compressor, CompressorBlockEntity.INPUT_SLOT, inputSlot.x(), inputSlot.y()));
        MachineGuiLayout.Element outputSlot = LAYOUT.outputSlots().get(0);
        addSlot(new Slot(compressor, CompressorBlockEntity.OUTPUT_SLOT, outputSlot.x(), outputSlot.y()) {
            @Override
    ...
    private static CompressorBlockEntity requireBlockEntity(Inventory inventory, FriendlyByteBuf buffer) {
        if (inventory.player.level().getBlockEntity(buffer.readBlockPos())
                instanceof CompressorBlockEntity compressor) {
            return compressor;
        }
        throw new IllegalStateException("Compressor block entity is missing");
    }

    @Override
    public boolean stillValid(Player player) {
        return stillValid(net.minecraft.world.inventory.ContainerLevelAccess.create(
                player.level(), ((CompressorBlockEntity) container).getBlockPos()),
                player, AflBlocks.COMPRESSOR.get());
    }
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        ...
        } else if (!moveItemStackTo(stackInSlot, CompressorBlockEntity.INPUT_SLOT,
                CompressorBlockEntity.INPUT_SLOT + 1, false)) {
