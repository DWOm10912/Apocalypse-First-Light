...
public class SupermarketShelfSingleBlockEntityRenderer implements BlockEntityRenderer<SupermarketShelfSingleBlockEntity> {
    private static final float ITEM_SCALE = 0.28F;
    private static final double FRONT_DEPTH = 0.21875D;
    ...
    public SupermarketShelfSingleBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
        this.itemRenderer = context.getItemRenderer();
    }

    @Override
    public void render(SupermarketShelfSingleBlockEntity shelf, float partialTick, PoseStack poseStack,
                       MultiBufferSource buffer, int packedLight, int packedOverlay) {
        if (shelf.getLevel() == null
                || shelf.getBlockState().getValue(SupermarketShelfSingleBlock.HALF)
        ...
        poseStack.mulPose(Axis.YP.rotationDegrees(getModelRotation(shelf.getBlockState().getValue(SupermarketShelfSingleBlock.FACING))));
        poseStack.translate(-0.5D, 0.0D, -0.5D);

        for (int slot = 0; slot < SupermarketShelfSingleBlockEntity.SIZE; slot++) {
