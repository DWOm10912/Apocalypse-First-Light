...
public final class MaintenanceFacingProbe {
    ...
    private static void shot(String suffix){if(mc().screen instanceof GunMaintenanceScreen){var s=MaintenanceModeClientState.INSTANCE;check(s.active(),"mode active");check(mc().gameRenderer.getMainCamera().getPosition().distanceTo(s.cameraPosition())<.01,"camera position "+FACES[index]);check(Math.abs(mc().gameRenderer.getMainCamera().getXRot()-67.5)<.01,"pitch");}Screenshot.grab(mc().gameDirectory,"maintenance_v2_"+FACES[index]+"_"+suffix+".png",mc().getMainRenderTarget(),m->{});}
