...
public final class NativeGunAmmoGameTests {
    ...
    public static void authoritativeFireAndRejectedReload(GameTestHelper h) {
        ...
        NativeGunActions.request(player, true, 0);
        NativeGunActions.request(player, false, 0);
        for (int i = 0; i < 20; i++) NativeGunActions.request(player, false, 0);
        ...
        for (int shot = 2; shot <= 5; shot++) {
            ...
            h.runAfterDelay((shot - 1) * 4, () -> {
                NativeGunActions.tick(new TickEvent.PlayerTickEvent(TickEvent.Phase.END, player));
                NativeGunActions.request(player, false, 0);
