...
RuralNaturalStructure.findGenerationPoint 先执行既有 Highway reservation 冲突检查；通过后 RuralNaturalGenerator 继续原有 site gate 和六资产加载，调用 RuralRoadNetwork.from → RuralFrontagePlanner.frontages → placements → RuralAccessPlanner.connect → 原有 lot terrain evaluation → 接纳 → 原有 RuralFarmPlanner.planBounded。
...
正式六资产 farmhouse_01、barn_large_01、house_small_01、storage_small_01、grain_silo_01、water_tower_01 全部使用同一 V2 geometry/access 路径；前五类消费现有合法 socket，water tower 无 socket 时 fallback。无新增资产，无 NBT/metadata/recipe 变更。
