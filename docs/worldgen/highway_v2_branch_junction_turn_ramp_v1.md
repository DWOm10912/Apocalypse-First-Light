...
- `HighwayRampRenderer`: finite claim + local ROW + ChunkOwned restriction. Parent median and inner lanes remain authoritative. At the branch opening, the final local surface/marking pass consumes the parent's competing edge concrete, parapet and paint; it restores only wrongly occupied drivable cells. White edges follow the union of the existing parent and ramp paint interiors, without a separate closed gore perimeter. TURN road/median/grade remain unchanged. Local cut/clearance stays inside the authorized envelope.
...
### Ramp Surface & Marking Cleanup V1

- 2026-09-21 修订：用户实机反馈否定了上一轮视觉完成结论。原来的 1161 项分类/邻接检查并未验证最终材质或模型接缝，不能作为修复验收。
- 混凝土真实生产链：`NaturalHighwayGenerationAdapter` 先执行主高速 `HighwayRenderer.placeOuterEdge/placeParapetsAndMedian`，再执行 Ramp。Ramp writer 原本排除 parent lateral <=11，而旧收尾只删除上层 slab，留下主高速先写的整块 reinforced concrete。另一个局部来源是 `HighwayGeometry.outerEdge` 把端口当边界，使 generic outer-edge 在端口 lateral=10 生成混凝土凸块。
- `finishSurfaceAndMarkings` 在 Junction 可行驶开口对最终写入仲裁：将内部误写的 reinforced concrete 恢复为 Asphalt，并移除该开口上方冲突的护边/标线。判定使用既有 parent/ribbon footprint 的内部邻接和虚拟轴向端口延续；保留真实外围结构、median、支撑与原始道路高度。Gore 底面仍为 Asphalt，无整层刷沥青。
- 白线圈的实际 producer 是旧 `goreEdge`：它给整个 gore 周界输出 `WHITE_EDGE`，邻接检查不会删除闭环。该周界生成分支已删除。Junction 白边线现沿 parent 与 Ramp 既有 paint interior 的合并边界连接到主高速。合流内 lateral<24 的短 divider 残段被局部抑制；最终先清除被接管单元的旧标线/step connector，再写唯一的局部标线。
- TURN 接口错位的实际 producer 差异：直路 `HighwayCorridor.buildRoadMarkings` 经 `HighwayRenderer.roadMarkingState` 输出 `connections=0`，白/黄线模型占方块边缘 14..16 像素；Ramp `placeLocalMarkings` 原输出中心型 `connections=1..16`，模型占 7..9 像素。旋转或补 connection bit 无法修复这 7 像素横移。
- 白/黄线增加局部边界型状态：`RoadMarkingBlock.connections=17..31` 表示 `16+NESW边界掩码`，复用现有直路 edge model；`33..47` 表示 `32+NE/SE/SW/NW凹角掩码`，用 2×2 像素 corner 补齐阶梯内角，32 保留不用。既有 0..16 状态及直路 producer 未改。只在 Ramp 写入新状态，端口实际形状与原轴向白/黄线一致。
- 资源：`src/main/resources/assets/apocalypse_firstlight/blockstates/edge_lane_{white,yellow}.json`；新增 `models/block/edge_lane_{white,yellow}_boundary_corner.json`。未修改 TURN asphalt/concrete edge/median、geometry、footprint、grade、elevation、RouteGraph、Claim、Routing、65×65 核心或 seam。
- 验证仅限 seed `-4332662446239654818` 的 Junction `(-961,-6504)` 和 W→N TURN `(-2592,-6504)`。测试在独立 JVM 绑定实际 AFL block suppliers，调用真实 placement passes、Ramp writer 与最终收尾，使用内存 BlockState 容器和固定高度夹具；未启动客户端、世界或 chunk 生成。检查最终 Asphalt、端口实际 shape 与直路一致、1/16 方块线型连通性、TURN 非标线材质保持不变。Junction 回放实际纠正 32 个 parent 混凝土单元；TURN 白线两条、黄线两条均连续。
- `compileJava` 与 `processResources` 通过。实机视觉验收仍由用户执行；此修改影响后续生成，已生成区块不会因重新编译自动重铺。未运行大规模测试、runClient、clean、GameTest、截图或其他 Highway 阶段，未 commit/push。

### Historical original V1 verification boundary (superseded by the scoped checks above)
