...
兼容范围不按配件名称自动推断：当前手枪消音器不能装 BR51，步枪消音器不能装 P9；BR51 尚无正式瞄具和弹匣配件兼容。未来步枪可通过自己的兼容声明与挂载点复用步枪消音器。当前扩容弹匣另有 `NativeMagazineItem.accepts` 的 P9 限制，不能仅改 JSON 就宣称支持其他枪。
...
当前四种配件均未引入额外伤害、射程、射速、散布、后坐力或换弹速度增减。不要从模型体积、重量感或现实配件效果推导出游戏数值。
