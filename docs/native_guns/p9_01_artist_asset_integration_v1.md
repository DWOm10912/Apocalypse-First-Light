...
| 空仓换弹 | `reload_empty` | 63 tick，结束提交后回 `static_idle` |
...
P9 原 gameplay 换弹长度 `1.30 秒/26 tick`、`1.65 秒/33 tick` 调整为作者动画的 `2.38 秒/48 tick`、`3.12 秒/63 tick`。仅这两项 gameplay timing 改变；17 发容量、9×19mm、射速 3 tick、伤害、射程、散布、后坐、Noise、附件兼容和 ADS 时间/FOV 均未改变。新资源换弹弹药提交在动画完成处；声音仍在作者 marker 原时间点，不人为改关键帧。
