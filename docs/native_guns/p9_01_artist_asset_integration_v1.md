...
P9 空仓重新装备修复：原 `action` 控制器在 draw trigger 期间不会执行空仓 `empty_idle` predicate，而作者 draw 自身把 `slide` 写成 loaded 位置。现于 action 之后注册 `empty_draw_slide` 控制器，仅在当前渲染 stack 为 0 发且 draw trigger 有效时播放原 `empty_idle` 的真实 `slide` 通道；其他空仓通道与 draw 身体/手部通道不被覆盖。shoot、reload、inspect 和 put-away 均不会启用这一层；结束后仍由原 P9 action predicate 选择 loaded/empty idle。`group2` 是资源孤立轨道，不参与覆盖。此改动只有动画显示层；首帧及联机视觉仍待客户端复验。
