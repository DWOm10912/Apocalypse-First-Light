...
On confirmation, tracer uses frozen origin → server endpoint, with visual start offset **0.20 blocks**, distance-clamped under existing trail rules. Trail lifetime/speed/profile and shader remain unchanged. Start never follows current muzzle. Bare muzzle flash uses the same frozen origin/direction for one presented world-render frame, then clears before subsequent recoiled frames (rather than retaining a detached one-tick flash core). Suppressed smoke spawns at that same origin. Network delay does not change the saved input pose. Suppressor resolver is the existing `NativeMuzzleRendering.applyExit`/muzzle_exit_anchor path. Casing handling remains on its existing path.
