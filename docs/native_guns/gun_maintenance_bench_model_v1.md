...
2026-09-09 状态更新：下文“配件未实现”属旧阶段描述，现已接入 P9 [附件交互 V1](attachments/gun_maintenance_attachment_interaction_v1.md)；维修仍未实现，本轮几何与贴图未改。

状态：美术资产已接入 `apocalypse_firstlight:gun_maintenance_bench` 四格静态方块。V2 删除维护垫上的两个黑/白装饰块；随后 Camera/Layout Polish 按用户圈图确认删除蓝色虎钳与桌面零件盒，维护垫宽度扩大 2 倍、深度 1.4 倍，贴图不改。详见 [布局打磨](gun_maintenance_camera_layout_transition_polish_v1.md)。
...
| 几何范围 | X：−8～24；Y：0～32；Z：0.45～16；16 模型单位为 1 格 |
| 组织 | 20 个分组，**392 cubes**；交接区域分件裁切，保留原材质 UV；维护垫无黑/白装饰零件 |
| 正面 / 原点 | 正面 NORTH（−Z），竖直 +Y；底部中心及组旋转枢轴 `[8,0,8]` |
| 材质 | 黑灰钢架/侧板、深灰挂板、棕木台面、青绿维护垫、军绿储物箱、灰工具箱与红盖清洁瓶；旧台钳/零件盒图集区域保留但不再使用 |
