...
> 当前追加 [手枪微型红点 V1](pistol_red_dot_v1.md)：P9 安装兼容 SIGHT 后瞄准红点中心，拆下恢复本页机械 profile。装拆仅通过枪械维护台，V 快捷装拆已移除。以下“无配件”指原机械 ADS 范围，不再描述完整现状。
...
| 参数 | BR51-01 | P9-01 |
| --- | --- | --- |
| ADS目标坐标（Gecko空间、未除16） | [0,13.6875,15.46875] | [1.80,5.60,2.97] |
| Eye relief | 0.16 | 0.34 |
| ADS scale | 0.45 | 0.41 |
| ADS rotation | [0,0,0] | [0,0,0] |
| ADS模型矩阵translation | [0,-0.3849609375,-0.59505859375] | [-0.046125,-0.1435,-0.41610625] |
...
BR51 HIP T=[3.8,-7.2,-11.5]/16 R=[0,4,0] S=.45；P9 HIP T=[3.24148,-7.4945,-14.19624]/16 R=[.54547,.19151,-.27948] S=.41，另有composition=[.10,.045,0]、静态 root pitch 补偿 3°。P9 左手 Display 也同步下移 2、远离镜头 5 模型单位。ADS 使用新 HIP 矩阵重新求逆到原后照门目标；左主手镜像补偿，左手模式尚未精校，列为已知限制。

定位矩阵仍由本类维护；ADS时间/FOV已迁移到单枪native_guns JSON，经 /reload 同步客户端。BR51三点共线、后照门距离、P9手臂遮挡仍以实机为准，不得仅凭矩阵称为对齐通过。

P9 Artist ADS 原有 X/Y 截图校准将目标设为 X=1.80、Y=5.60。现为修复 Hip 距镜头太近造成的手臂遮挡，将完整 Gun+Hands 的 P9 第一人称 Display Y 减 2、Z 减 5 模型单位，同时同步 `NativeAdsProfile.PISTOL` 的 HIP 参数；ADS 校正矩阵由新 HIP 反算，目标轴、eye relief、动画和弹道不变。此为数学上的重新校准，**新 Hip/ADS 画面尚待客户端验收**，不能将静态求解当作视觉通过。
